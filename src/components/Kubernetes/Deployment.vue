...
export default {
  ...
  methods: {
    constructJSON() {
      // this.integrateBase();// this.integrateTemplate();// this.integrateOther();// return JSON.stringify(this.deployment);
    ...
    integrate() {
      ...
      if (this.checkRequiredFields()) {
        ...
        this.integrateOther();
        this.content = json2yaml.stringify(this.deployment).replaceAll("\"", "");
      }
    },
    //检查必填项
    checkRequiredFields() {
      let metadata = this.$refs.base.integrateBase();
      ...
      if (flag) {
        this.$Message.error("Deployment对象中有尚未填写的必填项!");
        return false;
      }
      return true;
    },
    integrateBase() {
      this.deployment.metadata = this.$refs.base.integrateBase();
      this.deployment.metadata["name"] = this.resourceName;
    },
    integrateTemplate() {
      ...
      this.deployment.spec.template = template;
    },
    integrateVolumes() {
      ...
      if (commonUtil.isArrayEmpty(volumes)) {
        delete this.deployment.spec.template.spec.volumes;
      }else{
        this.deployment.spec.template.spec.volumes = volumes;
      }
    },
    integrateOther() {
      this.deployment.spec.replicas = this.$refs.otherTemplate.replicas;
      this.deployment.spec.selector.matchLabels = this.$refs.otherTemplate.integrateOtherTemplate();
    },
